...
LoopTp.MouseButton1Click:Connect(function()
    ...
    if moving then 
        moving = false 
        for i,mov in pairs(game:GetService("Workspace").Ignored:GetChildren())do
        ...
        local success, error = pcall(function()
            if game.Players.LocalPlayer.Backpack:FindFirstChild("[RPG]") == nil then
                repeat wait()
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Ignoredz.Shop["[RPG] - $6000"].Head.CFrame
                    fireclickdetector(game:GetService("Workspace").Ignoredz.Shop["[RPG] - $6000"].ClickDetector)
                until game.Players.LocalPlayer.Backpack:FindFirstChild("[RPG] - $6000") ~= nil
            end
            wait()
            if game.Players.LocalPlayer.Backpack["[RPG]"].Launcher:FindFirstChild("Mesh") then
                game.Players.LocalPlayer.Backpack["[RPG]"].Launcher.Mesh:Destroy()
            end
            wait(0.1)
            Plr.Character.Humanoid:EquipTool(Plr.Backpack["[RPG]"])
            Plr.Character["[Grenade]"]:Activate()
            wait(0.1)
            Plr.Character.HumanoidRootPart.CFrame = lastpos
            Plr.Character["[Grenade]"]:DeActivate()
            wait(0.1)
        end)
        wait(2)
        ...
        for index, part in pairs(game:GetService("Workspace").Ignored:GetChildren()) do
        	if part:IsA("Part") and part.Name == "Launcher" then
        ...
        repeat
        	for i,plr in pairs(players)do
        		for index, mover in pairs(movers) do
        		    ...
        		    game:GetService("Workspace").Ignored.Launcher.BodyVelocity.Velocity = -((game:GetService("Workspace").Ignored.Launcher.Position - TargetIns.Character.HumanoidRootPart.Position)/1.2)
        	    end
            end
            wait(0.5)
        until moving == false
